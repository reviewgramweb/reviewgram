...
def try_print_error(result, buffer):
...
def clone_or_update_repo(con, repoInfo, timestamp):
    ...
    credentials = ""
    if ((len(repoInfo["PASSWORD"]) != 0) and (len(repoInfo["USER"]) != 0)):
        credentials = repoInfo["USER"] + ":" + repoInfo["PASSWORD"] + "@"
    sitePath = credentials + repoInfo["REPO_SITE"]
